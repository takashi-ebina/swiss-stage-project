...
![対戦表](./public/assets/match.png)

#### プレイヤーが存在しない場合

![対戦表](./public/assets/match_no_player.png)

### ランキング

#### プレイヤーが存在する場合

![ランキング](./public/assets/ranking.png)

#### プレイヤーが存在しない場合

![ランキング](./public/assets/ranking_no_player.png)

### 参加者一覧

![参加者一覧](./public/assets/list.png)

### 設定

![設定](./public/assets/conf.png)
...
- 記載項目は下記
  ...
  - 名前　　※必須
  - 段級位　※必須
- 記載後、`参加者登録`を押下することで、対戦表に大会の参加者の内容が表示されます
- `参加者登録`押下時、対戦表への反映に加えて、以下2つの処理が実施されます
  - 段級位の高さ順に並び替えを行います
  - 参加人数が奇数の場合、`ダミーユーザー`が追加されます
- 【補足】
  - `CSV出力`から、参加者の一覧をCSVで出力できます
  - `CSV取込`から、参加者の一覧をCSVで取込できます
...
- 対戦結果は結果のセレクトボックスを選択してください
  - 結果は以下の3種類です
    ...
    - ×
    - △
- `対戦相手設定`を押下することで、1回戦～5回戦の対戦相手を自動で設定することができます
  - 何回戦に対して、対戦相手を設定するかは、`対戦相手設定`のセレクトボックスから選択できます
